...
var PhotoIDMatchProcessor = /** @class */ (function () {
    function PhotoIDMatchProcessor(sessionToken, sampleAppControllerReference, accessToken) {
        var _this = this;
        this.latestNetworkRequest = new XMLHttpRequest();
        this.accessToken = accessToken;
        this.cancelledDueToNetworkError = false;
        this.latestOrangeAPIResponse = null;
        //
        ...
        this.processSessionResultWhileFaceTecSDKWaits = function (sessionResult, faceScanResultCallback) {
            ...
            _this.latestNetworkRequest.open("POST", "https://xpapis.orange.jo/xyz/dev/v1/faceTech/enrollment-3d");
            _this.latestNetworkRequest.setRequestHeader("Authorization", "Bearer " + _this.accessToken);
            _this.latestNetworkRequest.setRequestHeader("Content-Type", "application/json");
            _this.latestNetworkRequest.setRequestHeader("Access-Control-Allow-Origin", "*");
            _this.latestNetworkRequest.setRequestHeader("Access-Control-Allow-Credentials", "true");
            _this.latestNetworkRequest.setRequestHeader("Access-Control-Allow-Methods", "GET");
            _this.latestNetworkRequest.setRequestHeader("Access-Control-Allow-Headers", "Content-Type");
            _this.latestNetworkRequest.onreadystatechange = function () {
                ...
                if (_this.latestNetworkRequest.readyState === XMLHttpRequest.DONE) {
                    try {
                        console.log("Orange Jordan API Response Status:", _this.latestNetworkRequest.status);
                        console.log("Orange Jordan API Response Text:", _this.latestNetworkRequest.responseText);
                        var responseJSON = JSON.parse(_this.latestNetworkRequest.responseText);
                        console.log("Orange Jordan API Response JSON:", responseJSON);
                        var scanResultBlob = responseJSON.scanResultBlob;
                        // In v9.2.0+, we key off a new property called wasProcessed to determine if we successfully processed the Session result on the Server.
                        // Device SDK UI flow is now driven by the proceedToNextStep function, which should receive the scanResultBlob from the Server SDK response.
                        // Handle Orange Jordan API response format
                        if (responseJSON.success === true && responseJSON.error === false && responseJSON.wasProcessed === true) {
                        ...
                        else if (responseJSON.error === true && responseJSON.errorMessage) {
                            // Handle Orange Jordan API error response
                            _this.cancelDueToNetworkError("Orange Jordan API Error: " + responseJSON.errorMessage, faceScanResultCallback);
                        }
                        else {
        ...
        this.processIDScanResultWhileFaceTecSDKWaits = function (idScanResult, idScanResultCallback) {
            ...
            //
            _this.latestNetworkRequest = new XMLHttpRequest();
            _this.latestNetworkRequest.open("POST", "https://xpapis.orange.jo/xyz/dev/v1/faceTech/match-3d-2d-idscan");
            _this.latestNetworkRequest.setRequestHeader("Authorization", "Bearer " + _this.accessToken);
            _this.latestNetworkRequest.setRequestHeader("Content-Type", "application/json");
            _this.latestNetworkRequest.setRequestHeader("Access-Control-Allow-Origin", "*");
            _this.latestNetworkRequest.setRequestHeader("Access-Control-Allow-Credentials", "true");
            ...
            _this.latestNetworkRequest.onreadystatechange = function () {
                ...
                if (_this.latestNetworkRequest.readyState === XMLHttpRequest.DONE) {
                    try {
                        console.log("Orange Jordan API ID Scan Response Status:", _this.latestNetworkRequest.status);
                        console.log("Orange Jordan API ID Scan Response Text:", _this.latestNetworkRequest.responseText);
                        var responseJSON = JSON.parse(_this.latestNetworkRequest.responseText);
                        console.log("Orange Jordan API ID Scan Response JSON:", responseJSON);

                        // Store the full Orange API response for later display
                        _this.latestOrangeAPIResponse = responseJSON;

                        var scanResultBlob = responseJSON.scanResultBlob;
                        // In v9.2.0+, we key off a new property called wasProcessed to determine if we successfully processed the Session result on the Server.
                        // Device SDK UI flow is now driven by the proceedToNextStep function, which should receive the scanResultBlob from the Server SDK response.
                        // Handle Orange Jordan API response format for ID scan
                        if (responseJSON.success === true && responseJSON.error === false && responseJSON.wasProcessed === true) {
                            ...
                            // Handle Orange Jordan API error response for ID scan
                            _this.cancelDueToNetworkError("Orange Jordan API Error: " + responseJSON.errorMessage, idScanResultCallback);
                        }
                        else {
                            // CASE:  UNEXPECTED response from API.  Our Sample Code keys off a wasProcessed boolean on the root of the JSON object --> You define your own API contracts with yourself and may choose to do something different here based on the error.
    ...
    // Getter method to access the latest Orange API response
    PhotoIDMatchProcessor.prototype.getLatestOrangeAPIResponse = function () {
        return this.latestOrangeAPIResponse;
    };
