...
app.post('/api/orange/enrollment-3d', async (req, res) => {
    ...
    try {
        ...
        console.error('Orange 3D enrollment error:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
});
...
app.post('/api/orange/match-3d-2d-idscan', async (req, res) => {
    ...
    try {
        ...
        console.error('Orange 3D-2D ID scan match error:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
});
...
// Start server
app.listen(PORT, () => {
    console.log(`Orange API Proxy Server running on port ${PORT}`);
